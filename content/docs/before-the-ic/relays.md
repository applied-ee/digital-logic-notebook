...
- **Relays are still the right answer for isolation and power** — mains switching, automotive loads, and safety interlocks routinely use a relay precisely because the control and load sides are galvanically separate and the contacts handle voltages and currents a small transistor cannot. This is not nostalgia; it is a live design choice.
...
- **The self-holding relay is the flip-flop's ancestor** — when a set/reset latch behaves unexpectedly, the seal-in relay is the clearest mental model of what "set dominant" or "reset dominant" actually means, because the priority is visible as which contact sits in series with the loop.
